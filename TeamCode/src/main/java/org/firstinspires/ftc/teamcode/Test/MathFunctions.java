...
public class MathFunctions {
    /**
     * Make sure an angle is within the range -180 to 180 degrees
     * @param angle
     * @return
     */
    ...
    public static ArrayList<Point> lineCircleIntersection(Point circleCenter, double radius, Point linePoint1, Point linePoint2) {
        ...
        double quadraticC = ((Math.pow(m1,2) * Math.pow(x1,2))) - (2.0*y1*m1*x1) + pow(y1,2) - pow(radius,2);
        ...
        try {
            ...
            if (xRoot2 > minX && xRoot2 < maxX) {
                allPoints.add(new Point(xRoot2, yRoot1));
            }
